...
22. 如果 Review 建议涉及具体代码修改、校验逻辑或测试补充，并且输入中有足够上下文，请在 `suggestedCode` 中给出一段可参考的修改后代码或测试代码；如果只能人工确认，返回空字符串。

23. `suggestedCode` 必须和同一条 Review 建议对应，优先基于 `changedFiles.patch` 中已有代码改写，不要编造不存在的 API、函数或业务对象。
